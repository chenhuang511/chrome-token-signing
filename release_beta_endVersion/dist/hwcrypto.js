/*! This is hwcrypto.js 0.0.10 generated on 2016-01-25 */
...
var hwcrypto = function hwcrypto() {
    ...
    function DigiDocPlugin() {
        ...
        this.sign = function(cert, hash, options, forcePkcs11, pkcs11ModulePath) {
            return new Promise(function(resolve, reject) {
                var cid = certificate_ids[cert.hex];
                if (cid) {
                    try {
                        var language = options.lang || "en";
						var forcePkcs11 = forcePkcs11.forcePkcs11 || "pkcs11";
						var pkcs11ModulePath = pkcs11ModulePath.pkcs11ModulePath || "null";
                        var v = p.sign(cid, hash.hex, language, forcePkcs11, pkcs11ModulePath);
                        resolve({
                            hex: v
                        });
                    } catch (ex) {
    ...
    function DigiDocExtension() {
        ...
        this.sign = function(cert, hash, options, forcePkcs11, pkcs11ModulePath) {
            return p.sign(cert, hash, options, forcePkcs11, pkcs11ModulePath);
        };
    }
    function NoBackend() {
    ...
    fields.sign = function(cert, hash, options, forcePkcs11, pkcs11ModulePath) {
        if (arguments.length < 2) return Promise.reject(new Error(INVALID_ARGUMENT));
        if (options && !options.lang) {
            options.lang = "en";
        }
		if (!forcePkcs11) {
            forcePkcs11.forcePkcs11 = "pkc11";
        }
        ...
        return _autodetect().then(function(result) {
            ...
            return _backend.sign(cert, hash, options, forcePkcs11, pkcs11ModulePath).then(function(signature) {
